...
class TeamSidebar:
    def __init__(self, parent, draft_callback, position_requirements):
        ...
        self.drafted_list = tk.Listbox(parent, selectmode="single", height=10,
                                       width=50)  # Add this if you want to display the drafted players
        self.drafted_list.pack(pady=10)  # Add this if you want to display the drafted players

    def add_player(self, player_data):
        player, position, par = player_data[1], player_data[2], player_data[3]  # Adjust based on your data structure
        self.drafted_players.append(player_data)
        self.drafted_list.insert(tk.END,
                                 f'{player} ({position}) - {par} PAR')  # Add this if you want to display the drafted players
    ...
    def remove_player(self, selected_index):
        ...
        position = player_data[1]  # Assuming position is at index 1
        del self.drafted_players[selected_index]
        self.drafted_list.delete(selected_index)  # Add this if you want to display the drafted players
        self.update_position_count(position, -1)
